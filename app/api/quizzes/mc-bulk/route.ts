...
export interface MCResponseItem {
  questionId: string;
  question: string;
  userAnswer: string;
...
interface BulkMCRequestBody {
  quizId: string;
  responses: MCResponseItem[];
}

// Updated extraction function (see above)
...
export async function POST(req: Request) {
  try {
    const { quizId, responses }: BulkMCRequestBody = await req.json();
    ...
    const completion = await openai.chat.completions.create({
      ...
      temperature: 0,
      messages: [
        { role: "system", content: "You are an expert evaluator for multiple choice quiz responses." },
    ...
    try {
      ...
      throw new Error("Failed to parse MC bulk evaluation result: " + responseText);
    }
  } catch (error) {
    console.error("MC Bulk Evaluation Error:", error);
    return NextResponse.json({ error: "Unknown error occurred in MC bulk evaluation." }, { status: 500 });
  }
}
